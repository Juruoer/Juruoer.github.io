...
rmf.switchDarkMode = function () {
    ...
    if (nowMode === 'light') {
        ...
        swalBar(' 已切换为深色模式！', '', 'success', 'top-end');
    } else {
        activateLightMode()
        saveToLocal.set('theme', 'light', 2)
        // GLOBAL_CONFIG.Snackbar !== undefined && btf.snackbarShow(GLOBAL_CONFIG.Snackbar.night_to_day)
        swalBar(' 已切换为浅色模式！', '', 'success', 'top-end');
    }
    // handle some cases
...
rmf.yinyong = function () {
    ...
    let inputValue = getSelection().toString().split('\n').join('\n> ');
    ...
    swalBar(' 引用成功! ', '若引用数学公式，建议复制后自行引用，不然会出现乱码！', 'success', 'top-end');
}
...
function popupMenu() {
    ...
    window.oncontextmenu = function (event) {
        if (event.ctrlKey || document.body.clientWidth < 768) return true;
